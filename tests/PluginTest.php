...
/**
 * Plugin validation tests for AppLifecycle.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('djurovicigoor/app-lifecycle');
        expect($manifest['namespace'])->toBe('AppLifecycle');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/AppLifecycleFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.djurovicigoor.plugins.app_lifecycle');
        expect($content)->toContain('object AppLifecycleFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/AppLifecycleFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum AppLifecycleFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/AppLifecycleServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Djurovicigoor\AppLifecycle');
        expect($content)->toContain('class AppLifecycleServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/AppLifecycle.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Djurovicigoor\AppLifecycle\Facades');
        expect($content)->toContain('class AppLifecycle extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/AppLifecycle.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Djurovicigoor\AppLifecycle');
        expect($content)->toContain('class AppLifecycle');
    });
});
